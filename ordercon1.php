...
$sql1="select * from consumable where id='$items'";
$result1=mysql_query($sql1);
while($row1=mysql_fetch_array($result1))
{   
   $qunt=$row1["quantity"]-$qty;
   $sql2=mysql_query("update consumable set quantity=$qunt where id=$items");
...
<table width="100%" border="0" align="center">
  <tr>
	...
	<a title="Save as PDF" href="html2pdf.php?<?php echo $a ?>" target="_blank" class="noPrint" style="text-decoration:none;color:#222;font-size:20px"><i class="fa fa-file-pdf-o"></i></a>&nbsp;
  ...
  <tr>
   ...
   <?php
	  ...
	  for($i=0;$i<count($item_name) && $i<count($quantity);$i++)
      {
       ...
       $items=substr($item,0,-2);
       $qty=$quantity[$i+1];
	   $sql1=mysql_query("select * from consumable where id='$items'");
